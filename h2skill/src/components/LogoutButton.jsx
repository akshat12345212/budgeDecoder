...
export const LogoutButton = ({ onClick }) => {
  ...
  const handleMouseEnter = () => {
    if (!isAnimating.current && animationState === 'default') {
      setAnimationState('hover');
    }
  };

  const handleMouseLeave = () => {
    if (!isAnimating.current && animationState === 'hover') {
  ...
  const handleClick = (e) => {
    ...
    setTimeout(() => {
      setExtraClasses(['clicked', 'door-slammed']);
      setAnimationState('walking2');
      setTimeout(() => {
        setExtraClasses(['clicked', 'door-slammed', 'falling']);
  ...
  const buttonStyle = logoutButtonStates[animationState];

  return (
    <button 
      className={`logoutButton logoutButton--dark ${extraClasses.join(' ')}`}
      style={buttonStyle}
